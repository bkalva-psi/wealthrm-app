/**
 * SWP (Systematic Withdrawal Plan) Form Component
 */
...
import { SWPFormData, Frequency } from '../types/systematic-plans.types';
import { Product } from '../types/order.types';

interface SWPFormProps {
  products: Product[];
  onSubmit: (data: SWPFormData) => Promise<void>;
  onCancel: () => void;
  isLoading?: boolean;
}

export default function SWPForm({ products, onSubmit, onCancel, isLoading = false }: SWPFormProps) {
  const [formData, setFormData] = useState<SWPFormData>({
    schemeId: 0,
    amount: 0,
    startDate: '',
    frequency: 'Monthly',
  ...
  const validate = (): boolean => {
    ...
    if (formData.amount < 1000) {
      newErrors.amount = 'SWP amount must be at least ₹1,000';
    }
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Create SWP Plan</CardTitle>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="amount">SWP Amount (₹) *</Label>
            <Input
              id="amount"
              type="number"
              min="1000"
            ...
            {selectedProduct && formData.amount > 0 && (
              <p className="text-sm text-muted-foreground">
                Ensure sufficient holdings in the scheme
              </p>
            )}
          </div>
          ...
          <div className="flex gap-2 justify-end">
            ...
            <Button type="submit" disabled={isLoading}>
              {isLoading ? 'Creating...' : 'Create SWP Plan'}
            </Button>
          </div>
        </form>
      </CardContent>
