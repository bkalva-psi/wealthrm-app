...
interface FixedTooltipChartProps {
  holdings?: { name: string; type: string; value: number; allocation: number; gain: number }[];
}

const FixedTooltipChart: React.FC<FixedTooltipChartProps> = ({ 
  holdings = [] 
}) => {
  ...
  const dataPoints = useMemo(() => processData(), [holdings]);
...
export default FixedTooltipChart;